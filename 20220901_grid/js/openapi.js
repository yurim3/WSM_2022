...
let url = `https://open.neis.go.kr/hub/mealServiceDietInfo` + `?KEY=${KEY}` + `&Type=json` + `&ATPT_OFCDC_SC_CODE=${ATPT_OFCDC_SC_CODE}` + `&SD_SCHUL_CODE=${SD_SCHUL_CODE}` + `&MLSV_YMD=${MLSV_YMD}`;
...
const urlToJSON = (url) => {
    ...
    //json 받아서 HTML 조식, 중식, 석식에 보여 주자
    const showmenu = (jsonString) => {
        console.log(jsonString)
        //jsonString -> json
        let json = JSON.parse(jsonString); //"{'key': 'value'}" -> 
